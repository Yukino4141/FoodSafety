...
后端开发请在 common/enumeration 包下定义这些枚举；前端请在常量文件中定义映射。

* **风险等级 (RiskLevel)**:  
  * 0: 安全 (Safe)  
  * 1: 中风险 (Medium) \- 如含糖高、含防腐剂  
  * 2: 高风险 (High) \- 如含反式脂肪酸、致敏原  
...
* **业务逻辑**:  
  1. 拼接 Prompt (提示词): "我是一名\[儿童\]，吃了含有\[植脂末, 代可可脂\]的食物，请分析健康风险，打分(0-100)，并给出建议。"  
  2. 调用 大模型 API (如文心一言/通义千问)。  
  3. 解析 AI 返回的文本。  
* **响应数据**:
